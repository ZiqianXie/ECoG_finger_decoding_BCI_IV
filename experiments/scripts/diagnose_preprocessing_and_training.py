...
from ecog_decoding.experimental.sequence import EcogTrajectoryDecoder
...
def ridge_audit(
    ...
    if not paper_train_path.exists() or not paper_test_path.exists():
        raise FileNotFoundError(
            "published baseline targets are missing; run experiments/scripts/prepare_paper_baseline_targets.py"
        )
...
def main() -> None:
    ...
    parser.add_argument("--config", type=Path, default=Path("experiments/configs/model.yaml"))
